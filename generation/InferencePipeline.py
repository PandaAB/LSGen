...
from utils.dataset_util import load_eval_dataset, extract_result
...
def parse_args() -> InferenceConfig:
    """Parse command line arguments and return a config instance. Command line arguments override config file values."""
...
def get_part_eval_dataset(data_list, local_dp_rank, dp_size, lock):
    # Calculate the amount of data each process handles
    data_per_rank = len(data_list) // dp_size
    start = local_dp_rank * data_per_rank
    end = start + data_per_rank
    if local_dp_rank == dp_size - 1:  # The last process handles the remaining data
        end = len(data_list)
    # Assign data from start to end in data_list to the current process, create a new list to avoid process deadlock
    new_data_list = []
    with lock:
...
class InferencePipeline:
    ...
    def run_in_parpallel(self):
        ...
        for each in res:
            ...
            single_data["gen_code1_bug_descriptions"] = each["gen_code1_bug_descriptions"]
...
if __name__ == "__main__":
    ...
    args = parse_args()  # Parse command line arguments and load config
    print(args)
    print("without_context:", args.without_context)
    inference_pipeline = InferencePipeline(args)
    inference_pipeline.run_in_parpallel()  # Start inference pipeline
    print("finished\n")
    #inference_pipeline.test_in_parallel()  # Start inference pipeline