...
from utils.dataset_util import load_eval_dataset, extract_result, extract_desc_result
...
class InferencePipeline:
    ...
    def generation(self, data_list, model_path, dp_size, tp_size, local_dp_rank, dp_master_ip, dp_master_port,
                ...
                use_lora, lora_path = None, barrier = None, debug_mode = True, batch_size = None):
        ...
        if use_lora:
            ...
            outputs = []
            cnt = 1
            for i in range(0, len(part_prompt_list), batch_size):
                print(f"Batch size number: {cnt}")
                batch_prompts = part_prompt_list[i : i + batch_size]
                batch_output = llm.generate(batch_prompts, sampling_params)
    ...
    def run_in_parpallel(self):
        ...
        for local_dp_rank in range(0, self.dp_size):
            ...
            proc = Process(target=self.generation,
                            args=(self.data_list, self.model_path, self.dp_size, self.tp_size,
                                  ...
                                  self.use_lora, self.lora_path, self.barrier, self.debug_mode, self.batch_size)
                            )
            proc.start()
            procs.append(proc)
        ...
        res = extract_desc_result(results, self.language)
        final_results = []
        for each in res:
            ...
            final_results.append(single_data) 
