...
def ensure_directory(file_path):
    """
    Check if the directory of the file path exists, and create it if it does not exist.
...
def save_list_to_parquet(data_list, file_path):
    """
    Store a list object to a Parquet file, one row per element.

    :param data_list: The list object to store
...
def read_parquet_to_list(file_path):
    ...
    :param file_path: Parquet file path
    :return: List read from the file
    """
    df = pd.read_parquet(file_path, engine='pyarrow')
...
def get_all_file_names(directory_path):
    """
    Get all file names in the specified directory path.
...
def load_list_from_jsonl(input_file_path):
    """
    Read a jsonl file and return a list, where each element is a JSON object.

    :param input_file_path: Path to the jsonl file
...
def save_list_to_jsonl(data_list, file_path):
    ...
    Args:
    - data_list: The list to save, each element should be a dictionary serializable to JSON.
    - file_path: The path to save the file, should end with .jsonl.
    """
