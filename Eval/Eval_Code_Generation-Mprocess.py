...
class EvalProcess:
    def __init__(self, 
        ...
        self.PsubmitStderrDict = {}

    # def SavePsubmitStderr2list(self, Psubmit, item):
        
    #     PsubmitStderr = []
    #     # for result in Psubmit.stderr
    ...
    def ProcessAllData(self):
        ...
        FinalResultDataList = []
        if self.CODE_KEY == "code1":
            for each in ResultDataList:
                if each['code_test_score'] != each['TotalScore']:
                    FinalResultDataList.append(each)
        else:
            FinalResultDataList = ResultDataList
        
        print(f" >>> The number of final result pairs are {len(FinalResultDataList)}.")

        save_data_to_json(FinalResultDataList, self.resotreFile_Path)
