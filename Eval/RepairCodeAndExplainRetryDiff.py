...
class RepairManager:
    def __init__(self, prompt_template: str, language: str, test_dataset_file: str, problem_description_file: str,
                 to_dir: str, save_file_name: str, num_thread: int = 10, model="gpt-4o-ca", topk=5, Retrieval_database="") -> None:
        ...
        with open(Retrieval_database, "r", encoding="utf-8") as f:
            retrieval_data = json.load(f)
        self.retrieval_data_dict = {item["submission1_id"]: item for item in retrieval_data}
    ...
    def _process_submission(self, submission_id: str) -> dict:
        ...
        retrieval_code_bug_desc = info["retrieval_code_bug_desc"]
        ...
        for entry in retrieval_code_bug_desc:
            retrieval_diff = self.retrieval_data_dict[entry["re_submission_id"]]["diff_code"]
            
            eb = entry['bug_description'].split("\n")
            eb1 = [f"# Each bug and its corresponding fix suggestion:"]
            for e in eb:
                eb1.append("# " + e)
            comment_desc = "\n".join(eb1)
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if repaired_code != "":
                        ...
                        print(f"\033[91mWarning: Repaired code is empty for {submission_id} on attempt {attempt}\033[0m")
                else:
...
You will receive one or more diff files. In these files, '-' marks lines deleted from the reference buggy code, and '+' marks lines added in the corresponding correct code, refer to them selectively.
[Programming Problem]:
 <TASK_DESCRIPTION>
[Buggy Code]: 
```python
...
""", help="prompt template")
    ...
    parser.add_argument("--Retrieval_database", type=str, default="dataset/Filtered_pair/code1_Added_testScode_pairs/ReDiff_Exec_test_processed_pair.json")
    ...
    RepairManager(prompt_template=args.prompt_template, 
                  ...
                  topk=args.topk,
                  Retrieval_database=args.Retrieval_database).start()