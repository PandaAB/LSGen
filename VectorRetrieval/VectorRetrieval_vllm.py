...
def retrieve_topk(query_vec, candidate_vecs, topk=5):
    ...
    # return topk_scores.squeeze(0).cpu().tolist(), topk_indices.squeeze(0).cpu().tolist()
...
def main(args):
    ...
    if args.model_type == "unixcoder":
        ...
        tokenizer = AutoTokenizer.from_pretrained("Qwen/Qwen3-Embedding-8B", trust_remote_code=True, padding_side="left")
        qwen_model = AutoModel.from_pretrained('Qwen/Qwen3-Embedding-8B', trust_remote_code=True).cuda()
        if args.gpus and len(args.gpus.split(",")) > 1:
    ...
    for item in tqdm(query_data, desc="Processing queries"):
        ...
        if args.mode == "code1+diff-code2":
            ...
            k = min(args.topk, sim_mat.size(-1))
            topk_scores, topk_indices = torch.topk(sim_mat.diag(), k)
        else:
            if args.mode == "code1-code1":
                candidate_codes = [c["code1"] for c in candidates]
            elif args.mode == "code1-code2":
                candidate_codes = [c["code2"] for c in candidates]
            else:
                raise ValueError(f"Unsupported mode: {args.mode}")

            query_vec = encoder([query_code])[0]
            candidate_vecs = encoder(candidate_codes)
            k = min(args.topk, len(candidate_vecs))
            topk_scores, topk_indices = retrieve_topk(query_vec, candidate_vecs, topk=k)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Code Retrieval")
    ...
    parser.add_argument("--mode", type=str, choices=["code1-code1", "code1-code2", "code1+diff-code2"], required=True, help="Retrieval mode")
    parser.add_argument("--model_type", choices=["unixcoder","qwen","qwen_vllm","infv1","infv1_vllm"], required=True, help="Choose encoding model")
